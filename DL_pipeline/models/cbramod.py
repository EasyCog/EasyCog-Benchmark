...
from DL_pipeline.models.criss_cross_transformer import TransformerEncoderLayer, TransformerEncoder
...
class CBraMod(nn.Module):
    def __init__(self, in_dim=200, out_dim=200, d_model=200, dim_feedforward=800, seq_len=30, n_layer=12,
                    nhead=8, load_pretrained_backbone=True):
        super().__init__()
        ...
        if load_pretrained_backbone:
            print('Loaded pretrained weights for CBraMod')
            self.load_state_dict(torch.load('/home/mmWave_group/EasyCog/DL_pipeline/models/CBraMod_pretrained_weights.pth',
                                     map_location='cpu'))
        else:
...
if __name__ == '__main__':
    ...
    model = CBraMod(in_dim=200, out_dim=200, d_model=200, dim_feedforward=800, seq_len=30, n_layer=12,
                    nhead=8).to(device)
    model.load_state_dict(torch.load('/home/mmWave_group/EasyCog/DL_pipeline/models/CBraMod_pretrained_weights.pth',
                                     map_location=device))
    a = torch.randn((8, 16, 3, 200)).cuda()
    b = model(a)
